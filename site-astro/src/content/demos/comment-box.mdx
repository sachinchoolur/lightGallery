...
<ImageGallery id="gallery-fb-comments-demo" photos="street" />
...
// The packages render the panel body from your own markup instead of
// injecting provider HTML strings.
<LightGallery
    slides={slides}
    plugins={[Comment]}
    comment={{
...
<template>
    ...
    >
        <template #comments="{ item }">
            <div class="fb-comments" :data-href="item.src" data-numposts="5" />
        </template>
    </LightGallery>
</template>
```
...
```html
<lg-gallery [slides]="slides" [features]="features" />

<ng-template #comments let-item>
    <div class="fb-comments" [attr.data-href]="item.src" data-numposts="5"></div>
</ng-template>
```

```ts
// features = [withComment({ commentBox: true, commentsTemplate })]
readonly commentsTemplate = viewChild<TemplateRef<unknown>>('comments');
```
...
<ImageGallery id="gallery-disqus-comments-demo" photos="morocco" />
